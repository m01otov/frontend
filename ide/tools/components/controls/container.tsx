...
import { containerFactory } from '../../factories';

type TDrawContainerToolProps = {} & TToolBaseProps;

export const DrawContainerTool: FC<TDrawContainerToolProps> = ({
  target,
  onComplete
}) => {
  ...
  const { isDrag, rect } = useMouseDragRect(target, (rect: TRectangle) => {
    ...
    const entityComponentsData = containerFactory({
      parent: {
        id: null
      },
      transform: {
...
DrawContainerTool.displayName = 'DrawContainerTool';

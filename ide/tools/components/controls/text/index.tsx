...
type TDrawTextToolProps = {} & TToolBaseProps;

export const DrawTextTool: FC<TDrawTextToolProps> = ({
  target,
  onComplete
}) => {
  ...
  const { isDrag, rect } = useMouseDragRect(target, (rect: TRectangle) => {
    ...
    const entityComponentsData = textFactory({
      parent: {
        id: null
      },
      transform: {
  ...
  return (
    <>
      <ToolFrame
        ...
        rect={rect}
        shouldRender={isDrag}>
        <label className={styles.draw_text_tool__preview}>Текст</label>
      </ToolFrame>
...
DrawTextTool.displayName = 'DrawTextTool';
